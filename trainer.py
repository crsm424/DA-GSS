...
def load_network(network, name):
    #save_path = os.path.join('/media/base/DG-Net-master/teacher/best/model',name,'net_last.pth')
    save_path = '/media/wang/Elements/DG-Net-master/teacher/best/model/100CUHK/net_last.pth'
    network.load_state_dict(torch.load(save_path))
    return network

def load_config(name):
    config_path = os.path.join('/media/wang/Elements/DG-Net-master/teacher',name,'opts.yaml')
...
hyperparameters['teacher_w'] * self.loss_teacher )  )
    ...
    def resume(self, checkpoint_dir, hyperparameters):
        ...
        #self.dis_a = nn.DataParallel(self.dis_a)
        state_dict = torch.load(last_model_name)
        self.dis_a.load_state_dict({k.replace('module.',''):v for k,v in state_dict['a'].items()})
        self.dis_b = self.dis_a
        # Load ID dis
