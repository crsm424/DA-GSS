...
parser.add_argument('--name', type=str, default='100CUHK', help="outputs path")
...
while True:
    for it, ((images_a,labels_a, pos_a),  (images_b, labels_b, pos_b)) in enumerate(zip(train_loader_a, train_loader_b)):
        ...
        #summary(trainer(images_a, images_b, pos_a, pos_b),input_size=(3,256,128),batch_size=1)
        print('Total params: %.2fM' % (sum(p.numel() for p in trainer.parameters()) / 1000000.0))
